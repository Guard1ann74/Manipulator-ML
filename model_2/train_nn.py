...
def main():
    ...
    print("\nЗагрузка языковой модели RuBERT-tiny2...")
    ...
    # --- СОХРАНЕНИЕ СТАТИСТИКИ (Исправлено) ---
    stats = {
        ...
        "report": report,
        # Сохраняем только тестовые эмбеддинги, чтобы они соответствовали y_test
    ...
    print("Статистика обучения сохранена.")

    # 7. Финальное сохранение модели (обучаем на всем датасете)
    clf.fit(X_all, all_labels_np)
    joblib.dump(clf, 'model_transform/intent_classifier.pkl')
    print("Модель переобучена на полном датасете и сохранена.")
